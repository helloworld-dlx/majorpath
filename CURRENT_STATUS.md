...
- **当前版本**：v0.22.4（AI解读主观题衔接）
- **更新日期**：2026-06-25 14:36
...
| 专业详情页 | ✅ 已完成 | 统一 11 模块模板 + 计算机类 + 电子信息类 + 集成电路类 + 机械类（完整 6 个）；另有 30 个专业类接入草稿简介（5 模块轻量版） |
...
| 测试报告页 | ✅ 已完成 | 纯原生 JS 引擎，10 模块完整渲染（画像/门类/专业类/避坑/建议/反馈/免责）+ 谨慎区改名+4层分级+卡片三区结构，无 AI API |
...
暂无。v0.22.0 正常。剩余 56 个专业类草稿待补充（见 TODO.md）。
...
| 模块 | 变更 |
|------|------|
| src/types/major-draft.ts | **新增** `MajorDraftContent` 类型（状态/一句话/学什么/高中关系/适合谁/注意/草稿提示 共 5 字段 + 草稿标记） |
| src/data/major-drafts.ts | **新增** 30 条草稿简介数据，自动从 `catalog` 反查 gateSlug/categorySlug 建立索引；**v0.22.1** 追加食品科学与工程类草稿（31→31条） |
| src/components/ui/MajorDraftTemplate.astro | **新增** 草稿渲染组件：草稿提示 + 5 模块卡片 + 共建入口 |
| src/pages/majors/[gate]/[category].astro | 内容优先级改为 `full > draft > building`；添加草稿标记标签；无草稿时保持现有 🚧 占位 |

## v0.22.3 模块变更

| 模块 | 变更 |
|------|------|
| src/data/questionBank.ts | 通用题6道：gen_002(B:协调→沟通调解, C:算预算→数字流程)、gen_004(A:想方向→拆清问题)、gen_007(B:推到结果→线索连成片, C:变化发生→现象真的出现)、gen_008(B:规则→约束边界, C:规则或原理→核心原理, D:试一下→拆搭动手, E:体验→感受心动)、gen_017(A:机械重复→看不到意义, B:一模一样→没有变化, D:照着安排→按模板, E:听别人→等点头)。方向题9道：br_stem_001A/003B/005A/009A/011AB/012B去代码/芯片/电路/算法术语，br_hum_005 A/B拉开构思vs改稿，br_soc_005去政策/执行抽象词，br_med_002标题去5-8年。避坑题2道：risk_005_D车间→操作间，risk_007_C判断框架→通俗表述。scoreEffects全部不变 |

## v0.22.4 模块变更

| 模块 | 变更 |
|------|------|
| functions/api/explain.js | system prompt 新增第6条红线：AI 解读时对用户主观题提到的方向做自然衔接（不添加为新的推荐、不推翻系统结果） |
| functions/api/explain.js | buildAIPrompt 主观题标签改为「用户在测试末尾填写的想法（供衔接参考）」 |
...
| 日期 | 更新内容 |
|------|----------|
| 2026-06-25 09:55 | **v0.22.2** 维度覆盖修复：life_health_interest 38→84分（+9处scoreEffects），business_sense 0→80分（+17处scoreEffects）；食品科学 weight 6→8；可选溢出降级到低优先。npm run build ✅ 116 页面 |
| 2026-06-25 14:36 | **v0.22.4** AI解读主观题衔接：API prompt 新增第6条规则，AI 解读时对用户主观题提到的方向做自然衔接，不推翻系统推荐。npm run build ✅ 116 页面 |
| 2026-06-25 13:00 | **v0.22.3** 题库表述优化：6道通用题选项区分度修复（gen_002/004/007/008/017），9道方向题去技术术语暴露（代码/电路/芯片/算法），1道方向题标题改（br_med_002），2道避坑题措辞微调（risk_005/007）。npm run build ✅ 116 页面 |
| 2026-06-24 23:15 | **v0.22.1** 食品科学与工程类草稿上线，建议学生更新文档后推送。npm run build ✅ 116 页面 |
| 2026-06-14 23:00 | **v0.22.0** 草稿简介系统上线：新增 MajorDraftContent 类型 + 30 条草稿数据 + MajorDraftTemplate 渲染组件 + 三级内容优先级（full > draft > building）。30个专业类从 🚧 变为可读状态。npm run build ✅ 116 页面 |
