...
### 2026-06-25 13:00 — 题库表述全面优化

- **使用模型**：opencode-go/deepseek-v4-pro
- **任务类型**：审查 / 内容
- **完成内容**：
  - 审查全部 20 道通用题 + 65 道方向题 + 8 道避坑题的表述质量（逻辑通顺度、高中生可读性、术语暴露）
  - **通用题** 6 道选项区分度修复：gen_002 B/C（协调vs数字）、gen_004 A（拆清问题）、gen_007 B/C（推理vs实验）、gen_008 B/C/D/E（约束边界/核心原理/拆搭动手/感受心动）、gen_017 A/B/D/E（无意义/无变化/按模板/等点头）
  - **方向题** 9 道修复：7 处 B4 理工方向去技术术语暴露（代码/芯片/电路/算法/数据结构/开源项目）、1 处 B2 社科去抽象表述（政策/执行层面）、1 处 B5 生命健康标题去 5-8 年
  - **避坑题** 2 道措辞微调：risk_005_D 车间→操作间，risk_007_C 判断框架→通俗表述
  - 全部改动仅改 user-visible text，scoreEffects/id/targetBuckets 零变更
- **修改的文件**：
  - `src/data/questionBank.ts` — 通用题 6 道 + 方向题 9 道 + 避坑题 2 道
  - `src/data/major-drafts.ts` — 食品科学与工程类草稿（前次会话）
  - `CURRENT_STATUS.md` — v0.22.3
  - `AI_WORKLOG.md` — 追加记录
- **关键决策**：
  1. gen_011 尝试改领域轴但不如原版程度轴清晰，回退
  2. br_stem_007_A（纯技术难题/算法更快/芯片更强）暂不动，后续有更好改法
  3. 校验题（cc_xxx）和 gen_010/013/020 的中等重叠问题暂不修，非硬伤
- **遗留问题**：
  - 剩余 55 个专业类无任何内容
  - gen_010 A/C 深度分析vs技术拆解仍有部分重叠
  - gen_020 C/D 动摇vs换方向程度差异仍存在
- **下次建议**：
  - 用户朋友完成测试后收集反馈
  - 继续补充高频专业类草稿
- **当前版本状态**：v0.22.3 — 题库表述全面优化，构建 116 页面通过

---
### 2026-06-25 14:36 — AI解读主观题衔接 + 项目状态整体回顾

- **使用模型**：opencode-go/deepseek-v4-flash
- **任务类型**：开发 / 文档
- **完成内容**：
  - 全面阅读项目六大文档（PROJECT_BRIEF/TECH_DECISIONS/CURRENT_STATUS/TODO/AI_WORKLOG/MODEL_HANDOFF）
  - git pull 同步 v0.22.3 变更（题库表述优化）
  - 分析 D老师提供的测试报告：食品科学不出现在推荐的根本原因（工学门类未进候选池）+ 推荐理由雷同 + 主观题信息未回流
  - 确认不修改推荐系统门类归属（避免单一 case 驱动的系统变更）
  - 方案B上线：functions/api/explain.js system prompt 新增第6条规则，AI 解读时对用户主观题提到的方向做自然衔接
  - 方案A（评分引擎层主观题加分）暂不实施，待方案B效果评估后再判断
  - 更新 CURRENT_STATUS.md → v0.22.4
- **修改的文件**：
  - `functions/api/explain.js` — system prompt 新增大白话第6条 + buildAIPrompt 主观题标签优化
  - `CURRENT_STATUS.md` — v0.22.2 → v0.22.4（含 v0.22.3）
  - `AI_WORKLOG.md` — 追加本记录
- **关键决策**：
  1. 不改推荐系统。食品科学不出现在生命健康画像中不是系统 bug，是门类归属的自然结果。
  2. 方案B（AI 解读衔接）优先于方案A（评分引擎加分），因为方案A的误触发风险（否定词/反话）不好解决。
- **遗留问题**：
  - 方案A 待评估：等方案B 上线后看真实用户反馈
  - food-science 跨门类引用（农学下加引用）暂不执行
- **下次建议**：
  1. 推送代码到 GitHub
  2. 如需微调 food-science 出现率，可考虑纯数据改动（不在农门下跨门类引用，而是调整工学下的 buckets 或 weight）
  3. 继续补充剩余 55 个专业类草稿内容
- **当前版本状态**：v0.22.4 — AI解读主观题衔接上线，构建 116 页面通过
