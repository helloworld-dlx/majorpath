...
${subjectiveNotes ? `▎用户在测试末尾填写的想法（供衔接参考）：${subjectiveNotes}` : ''}
...
6. 如果用户在主观题里提到了他想了解的方向（比如某个专业名），在解读中自然衔接一两句：这个方向跟推荐列表里的哪些领域有关、可以从推荐的哪个方向开始了解。如果用户提的不在推荐列表里，说一句它跟推荐方向的交叉关系即可。**不要把它加为新的推荐方向**，不要推翻系统结果。
